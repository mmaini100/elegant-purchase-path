...
const SearchPage = () => {
  const { searchTerm, setSearchTerm, setSearchActive } = useSearch();
  const [selectedCategory, setSelectedCategory] = useState(null);

  useEffect(() => {
    setSearchActive(searchTerm.trim().length > 0);
  }, [searchTerm, setSearchActive]);

const filteredProducts = searchTerm.trim()
  ? allProducts.filter(product =>
      product.name.toLowerCase().includes(searchTerm.toLowerCase())
    )
  : (selectedCategory ? data[selectedCategory] : allProducts);
  ...
  return (
    <div className="min-h-screen bg-white pt-10 px-4 flex max-w-7xl mx-auto gap-6">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="mb-8">
          <input
            ...
            className="w-full px-5 py-4 border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-black text-lg"
            autoFocus
          />
        </div>
        ...
        {searchTerm.trim() === '' && !selectedCategory ? (
          <div className="flex flex-col items-center justify-center flex-grow text-gray-400">
            ...
            <p className="text-xl italic">Your cart is empty. Start searching or select a category.</p>
          </div>
          ...
          <p className="text-center text-gray-500 italic flex-grow flex items-center justify-center">
            No products found matching <span className="font-semibold px-1">"{searchTerm}"</span>
          </p>
        )}
      </main>
    </div>
